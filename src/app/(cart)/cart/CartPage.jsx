  ...
  return (
    <>
      <Container maxW={'container.xl'}>
        <Flex m={4} p={8} flexDirection={'column'} gap={6} boxShadow={"rgba(0, 0, 0, 0.24) 0px 3px 8px;"}>
          {/* Item In Cart */}
          <ItemInCart />
          <ItemInCart />
        </Flex>
        <Flex ref={finalRef} justifyContent={'flex-end'} alignItems={'end'} gap={6} mt={'4rem'}>
          <Text as={'h6'} fontSize={'24px'} fontWeight={'bold'}>รวมสินค้าทั้งหมด</Text>
          <Text as={'h6'} fontSize={'34px'} fontWeight={'bold'} color={'#7F3A8A'}>฿39</Text>
          <Button bg={'#7F3A8A'} color={'white'} h={'3rem'} w={'13rem'} onClick={onOpen}>ซื้อสินค้า</Button>
        </Flex>
      </Container>
    ...
    <>
      <Modal size={'2xl'} isCentered ref={finalRef} finalFocusRef={finalRef} isOpen={isOpen} onClose={onClose}>
        ...
        <ModalContent>
          ...
          <ModalCloseButton />
          <ModalBody>
            <Flex px={'4rem'} flexDirection={'column'} justifyContent={'center'} gap={'5rem'}>
              <Flex gap={'2rem'} mt={4}>
                <Text as={'h6'} w={'5rem'}>กรณีเร่งด่วน </Text>
                <Flex gap={5}>
                  <Flex gap={2}>
                    <Box w={6} h={6}>
              ...
              <Flex flexDirection={'column'} gap={6}>
                ...
                <Flex gap={'2rem'} alignItems={'center'}>
                  <Text as={'h6'} w={'5rem'} fontWeight={'bold'} textAlign={'end'}>ข้อมูล</Text>
                  <Input w={'25rem'} placeholder='ชื่อ-นามสกุล' />
                </Flex>
                {/* ที่อยู่ */}
                <Flex gap={'2rem'} alignItems={'center'}>
                  <Text as={'h6'} w={'5rem'} fontWeight={'bold'} textAlign={'end'}>ที่อยู่</Text>
                  <Input w={'25rem'} placeholder='หากต้องการขายด่วน กรุณาติดต่อ 092-123-4567' />
                </Flex>
                {/* จังหวัด */}
                <Flex gap={'2rem'} alignItems={'start'}>
                  <Text as={'h6'} w={'5rem'} fontWeight={'bold'} textAlign={'end'}>จังหวัด</Text>
